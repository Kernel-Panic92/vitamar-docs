...
router.post('/', requireRol('admin'), async (req, res) => {
  const { nombre, jefe_nombre, email } = req.body;
  ...
  try {
    const { rows } = await db.query(
      ...
      [nombre.trim(), jefe_nombre?.trim() || null, email?.trim() || null]
    );
...
router.put('/:id', requireRol('admin'), async (req, res) => {
  ...
  try {
    const { rows } = await db.query(
      `UPDATE areas SET nombre=$1, jefe_nombre=$2, email=$3
       WHERE id=$4 AND activo=TRUE RETURNING *`,
      [nombre.trim(), jefe_nombre?.trim() || null, email?.trim() || null, req.params.id]
    );
    if (!rows[0]) return res.status(404).json({ error: 'Área no encontrada' });
    res.json(rows[0]);
  } catch (err) {
