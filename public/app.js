...
async function abrirF(id){
  ...
  showM(`Factura ${f.numero_factura}`,`
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:20px">
      ...
      <div style="background:var(--surface2);padding:12px;border-radius:8px"><div style="font-size:10px;color:var(--muted);text-transform:uppercase">Limite aprobacion</div><div style="font-weight:600;margin-top:4px;color:${f.limite_aprobacion&&new Date(f.limite_aprobacion)<new Date()?'var(--danger)':'var(--success)'}">${fdatetime(f.limite_aprobacion)}</div></div>
